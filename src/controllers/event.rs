...
pub async fn new(pool: &'static Pool, user_id: i64, event: NewEvent, headers: warp::http::HeaderMap) -> Result<impl Reply> {
    ...
    let time_running: u64 = headers.get("TIME_RUNNING").ok_or(Error::BadData)?.to_str().map_err(|_|Error::BadData)?.parse().map_err(|_|Error::BadData)?;
    let vcc: u16 = headers.get("VCC").ok_or(Error::BadData)?.to_str().map_err(|_|Error::BadData)?.parse().map_err(|_|Error::BadData)?;
    let free_heap: u64 = headers.get("FREE_HEAP").ok_or(Error::BadData)?.to_str().map_err(|_|Error::BadData)?.parse().map_err(|_|Error::BadData)?;
    ...
    let biggest_free_heap_block: u64 = headers.get("BIGGEST_FREE_HEAP_BLOCK").ok_or(Error::BadData)?.to_str().map_err(|_|Error::BadData)?.parse().map_err(|_|Error::BadData)?;
